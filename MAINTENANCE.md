...
| A6 | **In-house GitHub Action (pinned)** | `.github/workflows/check.yml` → `uses: thorvath-slower/seqtoid-ci-workflows/flake8-action@v1.1` | The flake8 action now lives in `seqtoid-ci-workflows` (collapsed from the standalone flake8-action repo); pinned to the immutable `v1.1` tag | Bump the pinned tag when a new `seqtoid-ci-workflows` release ships (CZID-204 / #405 / #406) |
