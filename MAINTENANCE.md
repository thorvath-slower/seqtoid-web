...
| A2 | **`Gemfile.lock` Ruby / BUNDLED WITH** | `Gemfile.lock` `RUBY VERSION` (`ruby 3.3.6p108`) and `BUNDLED WITH` (`2.5.22`). **Live drift:** `Dockerfile:83` installs `bundler -v '2.5.14'` | Written by `bundle`, not Renovate; the bundler pin is currently out of sync | Keep `Dockerfile:83` and `Gemfile.lock` BUNDLED WITH in sync; re-`bundle` after a Ruby bump |
