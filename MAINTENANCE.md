...
| A6 | **In-house GitHub Action (moving-tag SSOT)** | `.github/workflows/check.yml` → `uses: thorvath-slower/seqtoid-ci-workflows/flake8-action@v1` | The flake8 action lives in `seqtoid-ci-workflows` (collapsed from the standalone flake8-action repo); the `@v1` moving major tag is the SSOT — move it there to roll a new version out to every consumer at once | Move the `v1` tag in `seqtoid-ci-workflows` (CZID-204 / #405 / #407); no edit needed here |
