...
describe Sample, type: :model do
  ...
  describe "required / missing metadata fields" do
    let(:required_field) { create(:metadata_field, name: "req_field", is_required: 1) }
    let(:host_genome) { create(:host_genome, name: "MetaHost") }
    let(:sample) do
      project.metadata_fields << required_field
      host_genome.metadata_fields << required_field
  ...
  describe "#metadatum_validate" do
    it "returns no errors and no field for an unknown key" do
      sample = create(:sample, project: project)
